...
spacebot --debug start --foreground
...
spacebot --debug restart --foreground  # restart in foreground with debug
```

Logs go to `~/.spacebot/logs/` in daemon mode, or stderr in foreground mode.
...
<Cards>
  ...
  <Card title="GCP deployment" href="/docs/gcp-compute-engine" />
